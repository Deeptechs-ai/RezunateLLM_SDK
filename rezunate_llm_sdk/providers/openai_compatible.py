...
Used by any provider that speaks the OpenAI Chat Completions API
(``POST /chat/completions`` with OpenAI request/response shapes,
including SSE streaming). Subclasses just declare ``BASE_URL`` and
``PROVIDER``:
...
Authentication defaults to ``Authorization: Bearer <api_key>``. Override
``get_headers()`` if a particular provider needs a different scheme.
"""
